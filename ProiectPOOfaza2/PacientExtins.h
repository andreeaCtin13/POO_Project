...
public:
	...
	PacientExtins(const char* prenume, const string nume, const char* phoneNumber, const int varsta, const char adresa[], const bool eAsigurat, const bool areAlergii, string* boli, int nrBoli)
...
#pragma region Scriere_in_fisier
    friend ofstream& operator <<(ofstream& out, const PacientExtins& p) {
        if (p.getPrenume() != nullptr && p.getNume() != "") {
           ...
           // if (strlen(p.getAdresa()) > 10) {
             //   out << p.getAdresa() << endl;
            //}
            ...
            if (!(p.getNrConsultatii() == 0 || p.getViitoareConsultatie() == nullptr)) {
                out << p.getNrConsultatii() << endl;
